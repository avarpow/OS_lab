...
#include "program.h"
extern char key_buf[20];
extern char key_buf_size;
int times = 0;
int cur_x,cur_y;
        ...
        if (t == key_labels[i].value)
        {
            ...
            else
            {
                if (key_buf_size > 0)
                {
                    ...
                    printf("%c", key_labels[i].label_base);
                    //printf("size:%d\n", key_buf_size);
                    // uint8 temp = stdio.getCursor();
                    // printf("temp:%d\n", temp);
                    // cur_x=temp%80;
                    // cur_y=temp/80;
                    // stdio.moveCursor(cur_y,cur_x-1);
                    // stdio.print(' ');
                    // stdio.moveCursor(cur_y,cur_x-1);
                    // uint8 temp2 = stdio.getCursor();
                    //printf("x:%d y%d\n", x,y );
                }
            }
        }
