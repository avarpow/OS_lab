...
bool ProgramManager::copyProcess(PCB *parent, PCB *child)
{
    ...
    for (int i = 0; i < 768; ++i)
    {
        ...
        {
            // 无对应物理页
            pageTableVaddr[j]=pageTableVaddr[j]& 0xfffffffd;
            ...
            brk();
            asm_update_cr3(childPageDirPaddr); // 进入子进程虚拟地址空间
                pageTableVaddr[j] = pte & 0xfffffffd ;//直接复制父进程的页表
                // pageTableVaddr[j] = pte ;//直接复制父进程的页表
            //  pageTableVaddr[j] = (pte & 0x00000fff) | paddr;
            // memcpy(buffer, pageVaddr, PAGE_SIZE);
            // pageTableVaddr[j] = (pte & 0x00000ffd) | paddr;
            //memset(pageVaddr, 0, PAGE_SIZE);
